...
Mat Cropping(Mat large)
{
    ...
    /*adaptiveThreshold( grad, bw, 255, ADAPTIVE_THRESH_GAUSSIAN_C,THRESH_BINARY,3,0 ); //Threshold the gray
		...
		cvtColor(bw, bw, CV_RGB2GRAY);
		
bw.convertTo(bw,CV_8UC1, 255.0);
	//cout<<sobel.channels();
	cout<<(bw.type() == CV_8U)<<"channnels";
   ...
   // connect horizontally oriented regions
    ...
    // find contours
    Mat mask = Mat::zeros(grad.size(), CV_8UC1);
    vector<vector<Point>> contours;
    vector<Vec4i> hierarchy;
	imshow("imageee",connected);
	waitKey(0);
...
Mat background,mask;
    ...
    floodFill(src, cv::Point(0,0), Scalar(255));
	imshow("foreground",im_floodfill);
	waitKey(0);
	
/*	Mat destImage;

	cv::threshold(im_floodfill, background, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
	 background.setTo(cv::Scalar(0,0,255), mask);
	
	imshow("background",background);
	waitKey(0);
	im_floodfill.convertTo(im_floodfill, CV_8UC1);
	background.convertTo(background, CV_8UC1);
   alpha.convertTo(alpha, CV_8UC1, 1.0/255); // 
	
 
	cout<<im_floodfill.channels()<<alpha.channels();
	// Storage for output image
    Mat outImage = Mat::zeros(im_floodfill.size(), im_floodfill.type());
	imshow("outImage",outImage);
	waitKey(0);
	cout<<alpha.channels();
/* cvtColor(alpha, alpha, CV_RGB2GRAY);

//	cvtColor(alpha, alpha, CV_RGBA2BGRA);
	alpha.convertTo(alpha,CV_8UC1, 255.0);
	cv::threshold(alpha, alpha, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
		imshow("alpha12",alpha);


    // Multiply the foreground with the alpha matte
     multiply(alpha, im_floodfill, im_floodfill);  
	cout<<"alpha"<<alpha.channels();
	imshow("alphaa",alpha);
  // Multiply the background with ( 1 - alpha )
 // multiply(Scalar::all(1.0)-alpha, background, background); 
 
    // Add the masked foreground and background.
    add(im_floodfill, background, outImage); 
...
int main(int argc, char *argv[])
{
	...
	Mat outputImage;
	Mat img1=imread(image_dir,1);
	
/*
//Mat img1=imread("/home/artivatic/Desktop/23.jpg",1);
Mat img=negateImage(img1);
imshow("negate",img);
	//imwrite( "/home/artivatic/Desktop/negate.jpg", img );

	
// Sobel channels
Mat sobel1=sobelImage(img);
	imshow("sobel",sobel1);

		imwrite( "/home/artivatic/Desktop/sobel1.jpg", sobel1 );
	//waitKey(0);
	
Mat sobel;
cvtColor(img1, sobel, CV_RGB2GRAY);
sobel.convertTo(sobel,CV_8UC1, 255.0);

// Gaussian operator
Mat src=Gaussian(sobel1);

	
	imshow("gaussian",src);
	waitKey(0);

	Mat outImg=Cropping(src);
	*/
/*Mat finalImage=inFill(src,img1);
imshow("final_image",finalImage);
waitKey(0);
	*/
//Alpha mask 
		Mat img3=negateImage(img1);
